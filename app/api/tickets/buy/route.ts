...
export async function POST(request: NextRequest) {
  try {
    ...
    const { raffleId, discordId, quantity, signature, amount, wallet } = body;
    ...
    const existingTicket = await db
      .collection("tickets")
      .findOne({ tx_signature: signature });
    if (existingTicket) {
      return NextResponse.json(
    ...
    if (!tx) {
      ...
      return NextResponse.json(
        {
          error:
            "Transaction not confirmed yet. Please wait a moment and try again.",
        },
        { status: 404 },
      );
    }
    ...
    await db.collection("tickets").insertOne({
      ...
      wallet_address: wallet, // <-- CRITICAL: Store wallet address for winner selection
